...
uint16 g_preTrigBuff[PRE_TRIG_BUFF_SIZE_IN_WORDS];
uint16* g_startOfPreTrigBuff;
uint16* g_tailOfPreTrigBuff;
uint16* g_endOfPreTrigBuff;
uint16 g_maxEventBuffers;
uint16 g_currentEventNumber;
...
uint16 g_currentEventBuffer;
SUMMARY_DATA g_summaryTable[MAX_RAM_SUMMARYS];
SUMMARY_DATA* g_lastCompletedRamSummary;
