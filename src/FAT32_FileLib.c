...
static FL_FILE* _read_file(char *path)
{
	...
	if (FAT32_GetFileEntry(file->parentcluster, file->filename,&sfEntry))
	{
		...
		file->filelength =  ((UINT32)sfEntry.FileSize[0]);
		file->filelength += ((UINT32)sfEntry.FileSize[1] << 8);
		file->filelength += ((UINT32)sfEntry.FileSize[2] << 16);
		file->filelength += ((UINT32)sfEntry.FileSize[3] << 24);
		file->bytenum = 0;
		file->startcluster =  ((UINT32)sfEntry.FstClusHI[0] << 16);
		file->startcluster += ((UINT32)sfEntry.FstClusHI[1] << 24);
		file->startcluster += ((UINT32)sfEntry.FstClusLO[0]);
		file->startcluster += ((UINT32)sfEntry.FstClusLO[1] << 8);
