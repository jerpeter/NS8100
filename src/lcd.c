...
const uint8 SMALL_CHARACTERS[670] =
{
   ...
   0x78,0x85,0x87,0x84,0x48, // Ç 128
   0x1C,0x42,0x02,0x5C,0x02, // ü 129
   0x1C,0x6A,0xAA,0x2A,0x18, // é
   0x04,0x6A,0xAA,0x6A,0x1E, // â
   0x04,0xAA,0x2A,0xAA,0x1E, // ä
   0x04,0x2A,0xAA,0x6A,0x1E, // à
   0x02,0x55,0xB5,0x55,0x0F, // å
   0x38,0x45,0x47,0x44,0x28, // ç
   0x1C,0x6A,0xAA,0x6A,0x18, // ê
   0x1C,0xAA,0x2A,0xAA,0x18, // ë
   0x1C,0x2A,0xAA,0x6A,0x18, // è
   0x00,0x52,0x1E,0x42,0x00, // ï
   0x00,0x52,0x9E,0x42,0x00, // î
   0x00,0x92,0x5E,0x02,0x00, // ì
   0x0E,0x94,0x24,0x94,0x0E, // Ä
   0x07,0x4A,0xB2,0x4A,0x07, // Å
   0x3E,0x6A,0xAA,0x2A,0x22, // É
    0x24,0x2A,0x3E,0x2A,0x1A, // Backwards s with a line through it
   0x7E,0x88,0xFE,0x92,0x82, // Æ
   0x0C,0x52,0x92,0x52,0x0C, // ô
   0x0C,0x52,0x12,0x52,0x0C, // ö
   0x0C,0x12,0x92,0x52,0x0C, // ò
   0x1C,0x42,0x82,0x5C,0x02, // û 150
   0x1C,0x82,0x42,0x1C,0x02, // ù
   0x38,0x85,0x05,0x89,0x3E, //
   0x1C,0xA2,0x22,0xA2,0x1C, // Ö
   0x1C,0x42,0x02,0x42,0x1C, // Ü
...
//    temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
    if (lcd_register == COMMAND_REGISTER)
   {
      //temp &= 0xFE;
//      write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//      write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp & 0xFE);
//      lcd_port_image &= 0xFE;
      buffer[0] = temp;
   }
   else
   {
      //temp |= 0x01;
//      write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//      write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp | 0x01);
//      lcd_port_image |= 0x01;
      buffer[0] = temp;
    }
...
//      temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
        //temp &= 0xED;
//        write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//        write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp & 0xED);
//        lcd_port_image &= 0xED;
        buffer[1] = temp;
    }
   else
...
//      temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
       //temp &= 0xF5;
//	   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//      write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp & 0xF5);
//      lcd_port_image &= 0xF5;
      buffer[1] = temp;
   }
...
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//   temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp | 0x04);
//   lcd_port_image |= 0x04;
   buffer[2] = temp;

   //Write data
//   write_mcp23018(IO_ADDRESS_LCD, GPIOB, lcd_data);

   //Set E low
   temp &= 0xFB;
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//   temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp & 0xFB);
//   lcd_port_image &= 0xFB;
   buffer[3] = temp;
...
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp);
//   temp = read_mcp23018(IO_ADDRESS_LCD, GPIOA);
//   write_mcp23018(IO_ADDRESS_LCD, GPIOA, temp | 0x1B);
//   lcd_port_image |= 0x1B;
   buffer[4] = temp;
