...
void FATMisc_Create_sfn_entry(char *shortfilename, UINT32 size, UINT32 startCluster, FAT32_ShortEntry *entry)
{
	...
	entry->FstClusHI[1] = (BYTE)((startCluster>>24) & 0xFF);
	entry->FstClusHI[0] = (BYTE)((startCluster>>16) & 0xFF);
	entry->FstClusLO[1] = (BYTE)((startCluster>>8) & 0xFF);
	entry->FstClusLO[0] = (BYTE)((startCluster>>0) & 0xFF);
	entry->FileSize[0] = (BYTE)((size >> 24) & 0xFF);
	entry->FileSize[1] = (BYTE)((size >> 16) & 0xFF);
	entry->FileSize[2] = (BYTE)((size >> 8) & 0xFF);
	entry->FileSize[3] = (BYTE)((size >> 0) & 0xFF);
}
#endif
//-----------------------------------------------------------------------------
...
BOOL FATMisc_GenerateTail(char *sfn_output, char *sfn_input, UINT32 tailNum)
{
	...
	tail_str[0] = '~';
	sprintf(tail_str+1,"%d",(int)tailNum);
